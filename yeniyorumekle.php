...
if (isset($_GET["b"]) && (@$_SESSION['oturum'] == "acik")) {
    ...
    <div class="Yorumdiv">
        <form action="" method="post">
            <textarea name="yeniyorum" rows="8" cols="84px"
                      maxlength="535px"  <?php if ($banlimi['ban'] == 1) { ?> disabled placeholder="Ban Yedin Za.." style=" text-align:center ; font-size: 54px ;resize:none;"<?php } ?> ></textarea>
            <input type="submit" name="yeniyorumgeldi"
                   value="yolla" <?php if ($banlimi['ban'] == 1) { ?> disabled<?php } ?> />
        </form>
    </div>
    <?php
}
